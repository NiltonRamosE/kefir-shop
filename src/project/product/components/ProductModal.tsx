...
import { ShoppingCart, Check, X, Leaf, Droplets, Heart } from "lucide-react";
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold text-theme-ocean-twilight">
            {product.name}
          </DialogTitle>
        </DialogHeader>
              ...
              <div className="grid grid-cols-2 gap-2">
                {benefits.map((benefit, index) => (
                  <div key={index} className="flex items-center gap-2 text-sm">
                    <Leaf className="w-4 h-4 text-theme-maya-blue" />
                    <span>{benefit}</span>
                  </div>
                ))}
              </div>
